...
def main():
    ...
    output_filepath = args.output or f'output/dailyquote/{date:%Y}/{date:%Y%m%d}/dailyquote.csv'
    ...
    # if contract will delivery tomorrow, filter it out.
    ...
    for symbol in symbols:
        df_quote = bd.get_fapi_kline_1d(symbol, date)
        df_quote['symbol'] = symbol
        df_quote['date'] = date
        dfs.append(df_quote)

    df_dailyquote = pd.concat(dfs, ignore_index=True)
    if df_dailyquote.empty:
        print('no data', file=sys.stderr)
        sys.exit(1)

    columns = ['symbol', 'date', 'OpenTime', 'Open', 'High', 'Low', 'Close', 'Volume', 'CloseTime', 'Turnover', 'NumberOfTrades', 'TakerBuyVolume']
    df_dailyquote = df_dailyquote[columns]
    ...
    df_dailyquote.to_csv(output_filepath, index=False)

    time_end = time.time()

    print(f'{len(df_dailyquote)} rows written to {output_filepath}, {time_end-time_start:.3f}s')
...
"""
python -m rafdb.dailyquote --date=20220101
"""